...
export default function LeadersPage() {
  ...
  async function loadLeaders({ quiet = false } = {}) {
    ...
    try {
      ...
      // Hide departed players (kicked/left camp) from the board without
      // deleting their stats. Their player_totals rows stay in the DB.
      let visible = data || [];
      const ids = Array.from(new Set(visible.map((r) => String(r.player_id))));
      if (ids.length) {
        ...
        if (departedSet.size) visible = visible.filter((r) => !departedSet.has(String(r.player_id)));
      }
      setRows(visible);
    } catch (e) {
